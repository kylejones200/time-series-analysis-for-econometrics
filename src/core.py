...
def perform_cointegration_test(series1: pd.Series, series2: pd.Series) -> dict:
    ...
    return {'test_statistic': result[0], 'p_value': result[1], 'critical_values': result[2], 'is_cointegrated': result[1] < 0.05}
...
def plot_time_series(data: pd.DataFrame, title: str, output_path: Path, plot: bool=False):
    ...
    for col in data.columns:
        ax.plot(data.index if hasattr(data.index, '__len__') else range(len(data)), data[col], label=col, linewidth=1.2)
    ax.set_xlabel('Time')
    ax.set_ylabel('Value')
    ax.legend(loc='best')
