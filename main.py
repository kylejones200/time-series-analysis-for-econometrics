...
def main():
    parser = argparse.ArgumentParser(
        description="Time Series Analysis for Econometrics"
    )
    ...
    logging.info(
        f"{col}: ADF={result['adf_statistic']:.4f}, p-value={result['p_value']:.4f}, stationary={result['is_stationary']}"
    )
    if config["analysis"]["cointegration_test"]:
        result = perform_cointegration_test(data["x"], data["y"])
    logging.info(
        f"Cointegration: test_stat={result['test_statistic']:.4f}, p-value={result['p_value']:.4f}, cointegrated={result['is_cointegrated']}"
    )
